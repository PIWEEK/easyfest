<script lang="ts">
	...
	if (mapsMarkerMatch && mapsMarkerMatch.index !== undefined) {
		...
		contentAfterMap = rawContent.slice(mapsMarkerMatch.index + mapsMarkerMatch[0].length).trim();
	}
							...
							<div class="map-card">
								<div class="map-card__copy">
									...
									<p class="map-card__text">
										Podéis ampliar, reducir y desplazaros por el mapa. También podéis abrirlo
										directamente para preparar vuestra ruta.
									</p>
...
<style>
	.map-card {
		...
		background: linear-gradient(135deg, rgba(204, 234, 245, 0.85), rgba(255, 255, 255, 0.96));
